...
class CPYImageAndTextCell: NSTextFieldCell {
    ...
    override func copyWithZone(zone: NSZone) -> AnyObject {
        guard let cell = super.copyWithZone(zone) as? CPYImageAndTextCell else { return super.copyWithZone(zone) }
    ...
    override func titleRectForBounds(theRect: NSRect) -> NSRect {
        ...
        imageFrame.size = CGSize(width: 16, height: 16)
    ...
    override func drawWithFrame(cellFrame: NSRect, inView controlView: NSView) {
        ...
        if cellImageType == .Folder {
            ...
            NSImage(assetIdentifier: .IconFolder).drawInRect(imageFrame, fromRect: NSZeroRect, operation: .CompositeSourceOver, fraction: 1.0, respectFlipped: true, hints: nil)
        } else if cellImageType == .File {
            imageFrame.size = NSMakeSize(12.0, 13.0)
            NSImage(assetIdentifier: .IconText).drawInRect(imageFrame, fromRect: NSZeroRect, operation: .CompositeSourceOver, fraction: 1.0, respectFlipped: true, hints: nil)
